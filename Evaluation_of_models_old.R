...
################## create pdf file 
pdf("Evaluation_of_models19.pdf",width = 7, height=14, pointsize = 12, paper='special')
options(warn = -1) 
i <-1
...
data_log_regr$pred_target <- predict(object = model, type = "response")

# add values for perfomance plots(only for calc)
pred <- prediction(data_log_regr$pred_target,data_log_regr$target)
...
cdf1 <- ecdf(subset(data_log_regr$pred_target, data_log_regr$target == 1))
#point for plot(Bad Rate)
cdf2 <- ecdf(subset(data_log_regr$pred_target, data_log_regr$target == 0))
...
cM <- table(truth=data_log_regr$target,prediction=data_log_regr$pred_target> opt_cut[3,1]) 
...
data_log_regr$pred_target <- NULL
...
# first variant
#py - y pred
#dataset <- data_log_regr
loglikelihood <- function(y, py) {
  sum(y * log(py) + (1-y)*log(1 - py))
}

pnull <- mean(as.numeric(dataset[,-13]$target))
null.dev <- -2*loglikelihood(as.numeric(dataset[,-13]$target), pnull)

pred <- predict(model, type="response")
resid.dev <- -
  2*loglikelihood(as.numeric(dataset[,-13]$target), pred)

pr <- 1-(resid.dev/null.dev)
...
PRsquare = 1 - D/D.null
df <- with(model, df.null - df.residual)
p <- pchisq(D.null - D, df, lower.tail = FALSE) #like F-test in Lin regression
...
simple_fit <- glm(target ~ 1, data_log_regr, family = "binomial")
coef(simple_fit)
table(data_log_regr$target)
odds <- 3895 / 46263
log(odds) 
summary(simple_fit)
coef(model)
...
fit1 <- glm(target ~ Gender, data_log_regr, family = "binomial")
coef(fit1)
table(data_log_regr$target, data_log_regr$Gender)

table(data_log_regr$Gender)
