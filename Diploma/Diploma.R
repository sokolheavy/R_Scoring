...
Логарифмічна регресія -
 це різновид множинної регресії, 
призначення якої полягає в аналізі зв'язку між декількома незалежними змінними (предикторами) і залежною змінною.
 За допомогою бінарної логістичної регресії можна оцінити вірогідність події(повернення кредиту / дефолт).

Має деякі недоліки - низька стійкість до помилок, модель конколюється коефіцієнтами при змінних.

# http://datareview.info/article/10-tipov-regressii-kakoy-vyibrat/

Ласс-регресія  - це вдосконалення логарифмічної регресії з підвищеною стійкістю до помилок, 
що накладає обмеження на коефіцієнти регресії для отримання більш наближеного до реальності результату. 
Метод застосовується коли наявно велику к-сть даних, це часто призводить до того, що незалежні змінні корелюють один з одним (мультиколінеарность).

Лассо-регресія передбачає оцінку параметрів за такою формулою:

коефіцієнти регресії


Класифікація методом дерева рішень створює своєрідні блок-схеми для розподілу нових даних. 
У кожній точці блок-схеми задається питання про значимість того чи іншого параметра, і в залежності 
від цих параметрів він або вона [клієнти] потрапляють в певний клас. 

 Вузли дерева рішень можуть мати дві або більше гілок	
 Вузли рішення мають дві гілки
 різні методи відбору найбільш інформативних змінних
...
iv_table <- cbind.data.frame( variables = names(bin_data[variable_fc_bin:bin_ncol])
                    , IV = sapply(bin_data[variable_fc_bin:bin_ncol], function(x) round(IV(X=x, Y=bin_data$target_for_calc)[1],4))
                    , row.names = NULL) %>%
                      arrange(desc(IV)) %>%
...
bin_data$target_for_calc <- as.numeric(bin_data$target_for_calc)
...
bin_data$target_for_calc <- ifelse(bin_data$target_for_calc==1, 1, 0)
...
pdf("IV.pdf",width = 16, height=14, paper='special')
...
for (i in seq(1,(bin_ncol-variable_fc_bin), 50)){
  pdf(paste(pdf_name, "_part", ((i-1)/50)+1, ".pdf", sep = ''),width = 16, height=14, paper='special')
  for (j in (variable_fc_bin+i-1):min(variable_fc_bin+i+50-1,bin_ncol)) {
...
table(train$target_for_calc)
table(test$target_for_calc)
table(cdata_2$target_for_calc)


table(train$target_for_calc)[2]/table(train$target_for_calc)[1]
table(test$target_for_calc)[2]/table(test$target_for_calc)[1]
table(cdata_2$target_for_calc)[2]/table(cdata_2$target_for_calc)[1]


########################### Model1: Stepwise Logistic Regression Model
m1 <- glm(target_for_calc~., data=train, family=binomial())
m1 <- step(m1)
...
as.data.frame(colnames(data_log_regr))
# method to transform model: model2 <- update(model1, .~. - Age)
# Choose variables for different models:
groupvars = list()
groupvars[[1]] <- names(data_log_regr[,-1])[-8]
groupvars[[2]] <- names(data_log_regr[,-1])[c(1:6,9:10)]
groupvars[[3]] <- names(data_log_regr[,-1])[c(1:4,6,9:10)]
...
for (i in 1:nm) {
  ...
  pred_target <- as.data.frame(predict(object = model, type = "response"))
  
  # add values for perfomance plots(only for calc)
  pred <- prediction(pred_target, data_log_regr[,1])
