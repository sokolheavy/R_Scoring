...
sample_base <- as.data.table(dbGetQuery(con_k, "select * from risk_test.dbo.Scoring_DataMart_CC_base where productgroup = 'CC' and sample_type in ('test') and client_type = 'alfa'"))
...
select(total_sample, proc_Cash_ratio_ever)
...
#################################################################
#################################################################
setwd("S:/PROJECTS/! Individual/LSokol/!Rstudio/cc_test/trans/binning")
i <- 34
...
setwd("S:/PROJECTS/! Individual/LSokol/!Rstudio/cc_test/trans")
...
levels(bin_data$sum_amountbegin_loans_cat_sb)[levels(bin_data$sum_amountbegin_loans_cat_sb) == "(99;100]"] <- "(99;1500]"
levels(bin_data$sum_amountbegin_loans_cat_sb)[levels(bin_data$sum_amountbegin_loans_cat_sb) == "(100;1500]"] <- "(99;1500]"

levels(bin_data$sum_amountbegin_open_loans_cat_sb)[levels(bin_data$sum_amountbegin_open_loans_cat_sb) == "(-1;0]"] <- "(-1;800]"
levels(bin_data$sum_amountbegin_open_loans_cat_sb)[levels(bin_data$sum_amountbegin_open_loans_cat_sb) == "(0;800]"] <- "(-1;800]"

levels(bin_data$sum_amountmax_all_loans_cat_sb)[levels(bin_data$sum_amountmax_all_loans_cat_sb) == "(99;100]"] <- "(99;4500]"
levels(bin_data$sum_amountmax_all_loans_cat_sb)[levels(bin_data$sum_amountmax_all_loans_cat_sb) == "(100;4500]"] <- "(99;4500]"

levels(bin_data$max_amountbegin_all_loans_cat_sb)[levels(bin_data$max_amountbegin_all_loans_cat_sb) == "(99;100]"] <- "(99;1500]"
levels(bin_data$max_amountbegin_all_loans_cat_sb)[levels(bin_data$max_amountbegin_all_loans_cat_sb) == "(100;1500]"] <- "(99;1500]"
...
names(bin_data)
setwd("S:/PROJECTS/! Individual/LSokol/!Rstudio/cc_test/trans")
save(bin_data, file="bin_data_test.rda")
glimpse(bin_data)

grepl("", names(bin_data))
