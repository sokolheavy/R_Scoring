...
data<-read.csv2("CC_no_CSF_incall.csv",header=TRUE)
  str(data)
...
pdf("var_plot19.pdf",width = 12,height=15,paper='special')
for (j in 2:ncol(data)) {
  ...
  plot2_line<-ggplot(aggregate_table, aes(x=aggregate_table[,1],y=aggregate_table[,8], group = 1))+
    ...
    IV=round(sum((aggregate_table[,5]-aggregate_table[,6])/100*t2[,9]),4)
